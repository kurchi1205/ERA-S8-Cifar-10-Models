...
class NetLN(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv_block1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel),
            # nn.LayerNorm(normalized_shape=[self.base_channel, 30, 30]),
            nn.Dropout(0.02)
        )
        ...
        self.conv_block2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel),
            # nn.LayerNorm(normalized_shape=[self.base_channel, 28, 28]),
            nn.Dropout(0.02)
        )
        ...
        self.trans_block1 = nn.Sequential(
            nn.Conv2d(in_channels=self.base_channel, out_channels=self.base_channel * 2, kernel_size=(1, 1), padding=0, bias=False),
            nn.LayerNorm(normalized_shape=[self.base_channel * 2, 16, 16])
        )
        ...
        self.conv_block3 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel * 2),
            # nn.LayerNorm(normalized_shape=[self.base_channel * 8, 12, 12]),
            nn.Dropout(0.02)
        )
        ...
        self.conv_block4 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel * 2),
            # nn.LayerNorm(normalized_shape=[self.base_channel * 8, 10, 10]),
            nn.Dropout(0.02)
        )
        ...
        self.conv_block5 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel * 2),
            # nn.LayerNorm(normalized_shape=[self.base_channel * 8, 8, 8]),
            nn.Dropout(0.02)
        )
        ...
        self.trans_block2 = nn.Sequential(
            nn.Conv2d(in_channels=self.base_channel * 2, out_channels=self.base_channel * 4, kernel_size=(1, 1), padding=0, bias=False),
            nn.LayerNorm(normalized_shape=[self.base_channel * 4, 8, 8])
        )
        ...
        self.conv_block6 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=self.base_channel * 4),
            # nn.LayerNorm(normalized_shape=[self.base_channel * 16, 4, 4]),
            nn.Dropout(0.02)
        )
